/**
 * Module:   kthread
 * Purpose:  create a kernel thread
...
#define DRIVER_DESC   "Sample kthread test wh_kthread1"
#define PROCFS_MAX_SIZE		1024
#define PROCFS_NAME 		"wh_kthread1"
...
/** thread structure */
static struct task_struct *wh_kthread1_task;
...
/** test kernel thread */
int thread_wh_kthread1(void *arg) {
	...
	if (should_stop) {
	    printk(KERN_INFO "thread %s stopping\n", PROCFS_NAME);
	    break;
	}
	      ...
	      off_t offset, int buffer_length, int *eof, void *data)
{
	...
	wake_up_process(wh_kthread1_task);
...
int init_module()
{
	...
	/** create the thread */
	wh_kthread1_task = kthread_create(thread_wh_kthread1,
					  NULL,  PROCFS_NAME);
	wake_up_process(wh_kthread1_task);
...
void cleanup_module()
{
    ...
    /* stop thread */
    wake_up_process(wh_kthread1_task);
    res = kthread_stop(wh_kthread1_task);
